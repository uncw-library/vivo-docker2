        ...
        <ul id="browse-classes">
            <#list vClassGroup?sort_by("displayRank") as vClass>
                ...
                <#if (vClass.entityCount > 0)>
                    <li id="${vClassCamel}"><a href="#${vClassCamel}" title="${i18n().browse_all_in_class}" data-uri="${vClass.URI}">${vClass.name} <span class="badge bg-secondary float-end">${vClass.entityCount}</span></a></li>
                </#if>
            </#list>
        </ul>
